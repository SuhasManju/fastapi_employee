...
def email_generate(background_tasks:BackgroundTasks,email:EmailStr,emp:EmployeeOut):
    print(emp)
    ...
    message=f"OTP {email} to signup into your account {otp}"
    msg="Subject: "+subject+"\n"+message
    try:
    ...
    redis_client.hmset(emp.email,{'otp':otp,'data':json.dumps(emp.model_dump()),'attempts':0})
    redis_client.expire(emp.email,300)
